...
const CakeBenefitsCard: React.FC<React.PropsWithChildren<CakeBenefitsCardProps>> = ({ onDismiss }) => {
  ...
  const {
    ...
    <>
      <Text>
        {t(`%lockedCake% RICE (including rewards) are locked in the RICE Pool until %lockedEndTime%`, {
  ...
  const {
    ...
    <>
      <Text>
        {t(`iRICE allows you to participate in the IFO public sales and commit up to %iCake% amount of RICE.`, {
  ...
  const {
    ...
    <>
      <Text>{t(`bRICE allows you to boost your yield in PancakeSwap Farms by up to 2x.`)}</Text>
  ...
  const {
    ...
    <>
      <Text>
        {t(`vRICE boosts your voting power to %totalScore% in the PancakeSwap voting governance.`, {
        ...
        <>
          ...
          <Message mt="8px" mb="16px" variant="warning">
            <MessageText maxWidth="200px">
              {t(
                'Lock RICE to enjoy the benefits of farm yield boosting, participating in IFOs, voting power boosts, and so much more!',
              )}{' '}
              <NextLinkFromReactRouter
